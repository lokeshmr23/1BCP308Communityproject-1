...
try{
 ...
 await page.screenshot({path:'evidence/app-desktop.png',fullPage:true});
 check('home loads four main activities',await page.locator('.learn-card').count()===4);
 await page.locator('.primary[data-start="numbers"]').click();
 ...
 const download=page.waitForEvent('download');await page.locator('#export').click();await (await download).saveAs('evidence/example-practice.csv');check('CSV export works',fs.readFileSync('evidence/example-practice.csv','utf8').includes('false'));
 ...
 await navigate('learn');await page.locator('#language').click();check('Kannada language switch',await page.locator('html').getAttribute('lang')==='kn');await page.evaluate(()=>document.querySelector('#toast').textContent='');await page.screenshot({path:'evidence/app-kannada.png',fullPage:true});await page.locator('#language').click();
 await navigate('district');check('district map served',await page.locator('.map').evaluate(img=>img.complete&&img.naturalWidth>0));await page.screenshot({path:'evidence/app-map.png',fullPage:true});
 ...
 fs.writeFileSync('evidence/accessibility.json',JSON.stringify(a11y,null,2));check('automated a11y checks have no serious or critical findings',a11y.every(x=>x.violations.every(v=>!['critical','serious'].includes(v.impact))));
 await page.setViewportSize({width:390,height:844});await navigate('learn');await page.screenshot({path:'evidence/app-mobile.png',fullPage:true});check('mobile has no horizontal overflow',await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
 ...
 fs.writeFileSync('evidence/browser-results.json',JSON.stringify({browser:'Chromium 141 / Playwright 1.56.1',checks,errors,status:'PASS',note:'Automation only. Not school fieldwork or full accessibility certification.'},null,2));
